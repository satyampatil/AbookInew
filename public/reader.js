// --- READER PAGE LOGIC (WITH PRESENCE & GAMIFICATION) ---
...
// --- GAMIFICATION CONFIG (Must match Profile) ---
const BADGES = {
    books_read: {
...
function initializeReader() {
    ...
    // Inject Styles for Highlights & Badge Toast
    const style = document.createElement('style');
    style.innerHTML = `
        .word-highlight {
            background-color: #FFD700 !important; color: #000 !important;
            border-radius: 3px; box-shadow: 0 0 0 2px #FFD700;
            transition: background-color 0.1s; z-index: 10; position: relative; display: inline-block;
        }
        ...
        .badge-icon { font-size: 2.5rem; filter: drop-shadow(0 0 10px rgba(255, 215, 0, 0.5)); }
        .badge-info h4 { margin: 0; color: #FFD700; font-size: 0.9rem; text-transform: uppercase; letter-spacing: 1px; }
        .badge-info h3 { margin: 2px 0 0 0; color: #fff; font-size: 1.2rem; }
        .badge-info p { margin: 2px 0 0 0; color: #ccc; font-size: 0.85rem; }
    `;
    ...
    if (dropdownContainer && menuTrigger) {
        ...
        dropdownContainer.appendChild(pdfBtn);
        ...
        pdfBtn.addEventListener('click', () => {
            ...
            bookData.pages.forEach((pageText, index) => {
                ...
                htmlContent += `<div class="pdf-page" style="page-break-after: always;"><div style="font-size: 0.8em; color: #999; text-align: center; margin-bottom: 20px;">Chapter ${index + 1}</div>${formattedText}<div style="font-size: 0.8em; color: #ccc; text-align: center; margin-top: 30px;">- ${index + 1} -</div></div>`;
            });
            ...
            htmlContent += `
                <div class="pdf-page" style="padding-top: 50px; text-align: center;">
                    ...
                    <div style="margin-top: 40px; border-top: 1px solid #eee; width: 60%; margin-left: auto; margin-right: auto; padding-top: 30px;">
                        ...
                        <p style="font-size: 0.9em; color: #0077b5;">LinkedIn: linkedin.com/in/satyampatil</p>
                    </div>
                </div>
            `;
        ...
        saveCloudBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                // --- GAMIFICATION: TRACK PUBLISH ---
                await trackGamificationAction('books_published');
        ...
        saveListBtn.addEventListener('click', async () => {
            ...
            try {
                if (savedBookDocId) {
                    ...
                    const bookToSave = { ...cleanData, createdAt: serverTimestamp(), userId: currentUser.uid, originalUserId: originId };
    ...
    // --- GAMIFICATION LOGIC ---
    async function trackGamificationAction(actionType) {
        if (!currentUser) return;
        try {
            // Update stats
            const statsRef = doc(db, "users", currentUser.uid, "gamification", "stats");
            await setDoc(statsRef, { [actionType]: increment(1), lastUpdated: serverTimestamp() }, { merge: true });
            ...
            if (badgeMap && badgeMap[currentCount]) {
                await unlockBadge(badgeMap[currentCount]);
            }
    ...
    async function unlockBadge(badge) {
        ...
        await setDoc(userRef, {
            badges: arrayUnion({
                id: badge.id, name: badge.name, icon: badge.icon, desc: badge.desc, dateEarned: new Date().toISOString()
            })
        }, { merge: true });
    ...
    function updateButtonsState() {
        ...
        if (!bookData || !saveCloudBtn) return;
    ...
    function stopAudio() {
        ...
        if (activeHighlight) { activeHighlight.classList.remove('word-highlight'); activeHighlight = null; }
        if (activeParagraphHighlight) { activeParagraphHighlight.classList.remove('active-paragraph'); activeParagraphHighlight = null; }
    ...
    function speakNextParagraph() {
        ...
        if (spans.length === 0) { currentParagraphIndex++; speakNextParagraph(); return; }
        ...
        spans.forEach(span => {
            ...
            if (!text) return; 
            if (textToRead.length > 0) { textToRead += " "; cursor++; }
            const start = cursor;
        ...
        try {
            ...
            if (prefsJson) {
                const prefs = JSON.parse(prefsJson);
                if (prefs.voiceURI) voice = voices.find(v => v.voiceURI === prefs.voiceURI);
            }
        } catch (e) {}
        ...
        if (voice) {
            ...
            if (!voice.localService) {
                if (p) { activeParagraphHighlight = p; p.classList.add('active-paragraph'); }
            }
        }
        ...
        utterance.onboundary = (event) => {
            if (typeof event.charIndex === 'number') {
                ...
                if (match) highlightElement(match.element);
            }
        };
        utterance.onend = () => { if (isSpeaking) { currentParagraphIndex++; speakNextParagraph(); } };
        utterance.onerror = (e) => { if (e.error !== 'interrupted' && e.error !== 'canceled') stopAudio(); };
        synthesis.speak(utterance);
        if (p) p.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }

    function highlightElement(el) {
        if (activeHighlight) activeHighlight.classList.remove('word-highlight');
        activeHighlight = el;
    ...
    function updatePage() {
        ...
        if (currentPage === 0) {
            ...
            if(contentEl) contentEl.innerHTML = pHTML;
        }
        
        if(contentEl) paragraphsElements = Array.from(contentEl.querySelectorAll('.reader-paragraph'));
        if(pageIndicator) pageIndicator.textContent = `Page ${currentPage} of ${totalPages - 1}`;
        if(prevBtn) prevBtn.disabled = (currentPage === 0);
        if(nextBtn) nextBtn.disabled = (currentPage === totalPages - 1);

        // --- GAMIFICATION: BOOK READ ---
        if (currentPage === totalPages - 1 && !bookFinishedThisSession) {
            bookFinishedThisSession = true;
            trackGamificationAction('books_read');
        }
    }
